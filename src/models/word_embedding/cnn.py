    ...
    def _build_X_input(self, dataObj):
        ...
        for sentence in train_data.clean_text:
            ...
            for word in word_tokenize(sentence.lower()):
                try:
                    words.append(self.wordEmbedModel.word_vec(word))
                except KeyError: 
                    words.append(self.wordEmbedModel.word_vec('<unk>'))
                except AttributeError:
                    print(sentence)
            sentence_vectors.append(words)

        normalized_input = [np.concatenate((sentence,np.zeros((self.max_len - len(sentence),self.n_features_per_word)))) for sentence in sentence_vectors]
